...
public class PaintSquare {
    ...
    public static void main(String[] args) throws IOException {
        // PaintSquare square = loadAllTracksr(Paths.get("/Users/hans/Downloads/221012/All Tracks.csv"),"221012-Exp-1-A1-2-threshold-8");

        // PaintSquare square = loadTracksr(Paths.get("/Users/hans/Downloads/221012/All Tracks.csv"),"221012-Exp-1-A1-2-threshold-8");
    }

    public class PaintTrackLoader {

        List<PaintTrack> loadTracks(Path csvPath) throws Exception {
            Table table = Table.read().csv(csvPath.toFile());
            List<PaintTrack> tracks = new ArrayList<>();

            for (Row row : table) {
                PaintTrack track = new PaintTrack(
                        String.valueOf(row.getInt("Track Id")),   // or getString if not strictly int
                        row.getString("Track Label"),
                        row.getInt("Nr Spots"),
                        row.getInt("Nr Gaps"),
                        row.getDouble("Longest Gap"),
...
//    public PaintSquare(Path path, String recordingName) throws IOException {
//
...
//        System.out.println(struct.print());
//
//        // Filter on Process == "Yes"
//        StringColumn recordingNameCol = table.stringColumn("Ext Recording Name");
//        Table filteredTable = table.where(recordingNameCol.isEqualTo(recordingName));
//        System.out.println(filteredTable.rowCount());
//        System.out.println(filteredTable.first(5).print());
//
